...
# Middleware to ensure user is authenticated and authorized based on their role
def role_required(role):
    if 'uid' not in session or session['role'] != role:
        return jsonify({'error': 'Unauthorized'}), 403

# Route for module organizers to create assignments
...
def create_assignment():
    ...
    feedback_form_id = data['feedback_form_id']  # Link to the feedback form created by module organizer
...
# Route to create feedback forms (for module organizers)
